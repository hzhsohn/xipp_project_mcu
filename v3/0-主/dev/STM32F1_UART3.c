...
void STM32F1_UART3_Init(u32_t lBaudRate)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);   //enable clock of GPIOB
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3| RCC_APB2Periph_AFIO, ENABLE); //enable clock of USART3
		GPIO_PinRemapConfig(GPIO_FullRemap_USART3,ENABLE);
	
    /* Configure USART Tx as alternate function push-pull */
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;//ISART3_TX PB10
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;//复用推挽输出
    GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化 
	
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;//USART3_RX  PB11
